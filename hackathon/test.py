uthor: Aaron Iniguez and Sabrna Matsunaga
import ast
import glob
import os
import optparse
...
def get_num_of_agents(hw_details,filename="HWDetails.log", qemuPorts=0):
    with open(os.path.basename(filename), 'w') as f:
        ...
        for chassis in hw_details:
            ...
            f.write('-'*60 + '\n\n')
        totalportpairs = totalportpairs+qemuPorts/2
...
def getJenkinsParametersPortPairs():
    if "CI_HW_MODULES" in os.environ:
        if os.environ['CI_HW_MODULES'] == "stcv-qemu":
            portPairs = ast.literal_eval(os.environ['VIRTUAL_SERVER_OPTIONS'].replace("\"",""))['max_port_pairs']
            return portPairs
    return 0
...
def main():
    ...
    for key,value in vars(options).iteritems():
        print key, ":", value
    print"************************************\n"
    qemuPorts = getJenkinsParametersPortPairs()
    if not options.resource_file and not options.hw_detail and qemuPorts == 0:
        ...
        if containsQemu(options.resource_file):
            portPairs = ast.literal_eval(os.environ['VIRTUAL_SERVER_OPTIONS'].replace("\"",""))['max_port_pairs']
            numberAgents = get_num_of_agents(parse_resourcefile(options.resource_file),filename, portPairs)
        else:
            numberAgents = get_num_of_agents(parse_resourcefile(options.resource_file),filename)
        return numberAgents
    elif options.hw_detail:
        return get_num_of_agents(options.hw_detail, qemuPorts = qemuPorts)
    else:
        return convert_num_agents_req(qemuPorts)
